...
bool Sample::Frame()
{
	...
	if (g_Input.GetKey(VK_RBUTTON) == KEY_PUSH)
	{
		...
		//	}
		//}
		for (int i = m_Map->m_VertexList.size() / 4; i < m_Map->m_VertexList.size() * 3 / 4; i++)
		{
			m_Map->m_VertexList[i].p.y += 5.0f;
		}
		g_pImmediateContext->UpdateSubresource(m_Map->m_pVertexBuffer.Get(), 0, NULL,
			&m_Map->m_VertexList.at(0), 0, 0);
	}
