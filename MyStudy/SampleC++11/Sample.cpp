...
//pArray[0] = 1;
//pArray[1] = 2;
//std::unique_ptr<int[]> pCopyArray;
//pCopyArray = std::move(pArray);
									//= pArray(new stStruct[10]);
//std::shared_ptr<stStruct> pArray = std::make_shared<stStruct>();
//pArray->k = 1;
//shared_ptr 안에 Get이라는 함수는 원본의 포인터를 반환한다
//->로 접근하면 Get을 호출
//move는 포인터값을 이동시킴
