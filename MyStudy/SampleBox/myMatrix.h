...
class myMatrix : public float4x4
{
public:
...
public:
	...
	myMatrix operator * (myMatrix const &Matrix)
	{
		myMatrix OutMatrix;
	...
	myVector3 operator * (myVector3 const &vector)
	{
		return myVector3(
	...
	void Translation(const myVector3 &vector)
	{
		_41 = vector.x;
		_42 = vector.y;
		_43 = vector.z;
	...
	void Scale(const myVector3 &vector)
	{
		_11 = vector.x;
		_22 = vector.y;
		_33 = vector.z;
	...
	//           뷰 행렬 생성 
	//---------------------------------------------------------
	myMatrix ViewLookAt(myVector3& vPosition,
		myVector3& vTarget,
		myVector3& vUp)
	{
		myMatrix matrix;
		myVector3 vDirection = (vTarget - vPosition).Normal();
		myVector3 vRightVector = (vUp ^ vDirection).Normal();
		myVector3 vUpVector = (vDirection ^ vRightVector).Normal();
	...
	myMatrix CreateViewLook(myVector3& vPosition, myVector3& vTarget, myVector3& vUp)
	{
		myMatrix matrix;
		myVector3 vDirection = vTarget - vPosition;
		vDirection = vDirection.Normal();
		float fDot = vUp | vDirection;
		myVector3 vUpVector = vUp - (vDirection * fDot);
	...
	//        |    ---------------->
	// 내적의 결과는 직각이 된다.
	void ObjectLookAt(myVector3& vPosition,
		myVector3& vTarget,
		myVector3& vUp)
	{
		myVector3 vDirection = vTarget - vPosition;
	...
	myMatrix	InverseGeneral(float *out = NULL);
	myMatrix	Inverse(float *out = NULL);

	// 뷰 행렬 계산
	myMatrix ComputeViewMatrix(myVector3& vPosition,
		myVector3& vTarget,
		myVector3& vUp)
	{
		myMatrix matrix;
	...
	// 원근 투영 행렬 계산
	// 원근 투영 행렬 계산
	myMatrix PerspectiveFovLH(float fNearPlane,
		float fFarPlane,
		float fovy,
