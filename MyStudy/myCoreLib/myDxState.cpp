#include "myDxState.h"
ID3D11RasterizerState* myDxState::g_pRSWireFrame = 0;
ID3D11RasterizerState* myDxState::g_pRSSolid = 0;
ID3D11RasterizerState* myDxState::g_pRSBackCullSolid = 0;
ID3D11RasterizerState* myDxState::g_pRSNoneCullSolid = 0;
ID3D11RasterizerState* myDxState::g_pRSFrontCullSolid = 0;
ID3D11RasterizerState* myDxState::g_pRSSlopeScaledDepthBias;
ID3D11RasterizerState* myDxState::g_pRS[6] = { 0, };

ID3D11SamplerState*	 myDxState::g_pTexSS = 0;
ID3D11SamplerState*  myDxState::g_pSSWrapLinear = NULL;
ID3D11SamplerState*  myDxState::g_pSSWrapPoint = NULL;
ID3D11SamplerState*  myDxState::g_pSSMirrorLinear = NULL;
ID3D11SamplerState*  myDxState::g_pSSMirrorPoint = NULL;
ID3D11SamplerState*  myDxState::g_pSSClampLinear = NULL;
ID3D11SamplerState*  myDxState::g_pSSClampPoint = NULL;
ID3D11SamplerState*  myDxState::g_pSSShadowMap = NULL;
ID3D11SamplerState*  myDxState::g_pSS[8] = { 0, };

ID3D11DepthStencilState*	myDxState::g_pDSSDepthEnable = 0;
ID3D11DepthStencilState*	myDxState::g_pDSSDepthDisable = 0;
ID3D11DepthStencilState*	myDxState::g_pDSSDepthEnableNoWrite = 0;
ID3D11DepthStencilState*	myDxState::g_pDSSDepthDisableNoWrite = 0;
ID3D11DepthStencilState*	myDxState::g_pDSSDepthStencilAdd = 0;
ID3D11DepthStencilState*	myDxState::g_pDSSDepthAlways = 0;
ID3D11DepthStencilState*	myDxState::g_pDSS[6] = { 0, };

ID3D11BlendState*	  myDxState::g_pAlphaBlend = 0;
ID3D11BlendState*	  myDxState::g_pNoAlphaBlend = 0;
ID3D11BlendState*	  myDxState::g_pBSColorOne = 0;
ID3D11BlendState*	  myDxState::g_pBSOneOne = 0;
ID3D11BlendState*	  myDxState::g_pBSOneZero = 0;
ID3D11BlendState*	  myDxState::g_pBSAlphaOne = 0;
ID3D11BlendState*	  myDxState::g_pBSMaxOneOne = 0;
ID3D11BlendState*	  myDxState::g_pBS[7] = { 0, };

Microsoft::WRL::ComPtr<ID3D11BlendState>		myDxState::g_pBSEdit = 0;
Microsoft::WRL::ComPtr<ID3D11RasterizerState>	myDxState::g_pRSEdit = 0;
Microsoft::WRL::ComPtr<ID3D11SamplerState>      myDxState::g_pSSEdit = 0;
Microsoft::WRL::ComPtr<ID3D11DepthStencilState> myDxState::g_pDSSEdit = 0;
D3D11_BLEND_DESC			myDxState::g_BlendDesc = { 0, };
D3D11_RASTERIZER_DESC		myDxState::g_RasterizerDesc;
D3D11_SAMPLER_DESC			myDxState::g_SamplerDesc;
D3D11_DEPTH_STENCIL_DESC	myDxState::g_DepthStencilDesc = { 0, };

FLOAT myDxState::g_fBlendFactor[4] = { 0, };
UINT  myDxState::g_iMask = 0xff;
UINT myDxState::g_iSlot = 0;
UINT myDxState::g_iArray = 1;
UINT myDxState::g_iRef = 1;

HRESULT myDxState::SetBlendState(ID3D11Device* pd3dDevice,
...
HRESULT myDxState::SetRasterizerState(ID3D11Device* pd3dDevice,
	ID3D11DeviceContext* pContext,
...
HRESULT myDxState::SetSamplerState(ID3D11Device* pd3dDevice,
	ID3D11DeviceContext* pContext,
...
HRESULT myDxState::SetDipthStencilState(ID3D11Device* pd3dDevice,
	ID3D11DeviceContext* pContext,
...
HRESULT myDxState::SetState(ID3D11Device*	pd3dDevice)
{
	...
	pd3dDevice->CreateBlendState(&blendDesc, &myDxState::g_pBSColorOne);

	blendDesc.RenderTarget[0].BlendOp = D3D11_BLEND_OP_ADD;// D3D11_BLEND_OP_REV_SUBTRACT;
	blendDesc.RenderTarget[0].SrcBlend = D3D11_BLEND_ONE;// DEST_COLOR;// D3D11_BLEND_DEST_COLOR;
	blendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_ONE;// DEST_COLOR;
	pd3dDevice->CreateBlendState(&blendDesc, &myDxState::g_pBSOneOne);

	blendDesc.RenderTarget[0].SrcBlend = D3D11_BLEND_ONE;// DEST_COLOR;// D3D11_BLEND_DEST_COLOR;
	blendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_ZERO;// DEST_COLOR;
	pd3dDevice->CreateBlendState(&blendDesc, &myDxState::g_pBSOneZero);

	blendDesc.RenderTarget[0].SrcBlend = D3D11_BLEND_SRC_ALPHA;
	blendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_ONE;
	pd3dDevice->CreateBlendState(&blendDesc, &myDxState::g_pBSAlphaOne);

	blendDesc.AlphaToCoverageEnable = TRUE;
	blendDesc.RenderTarget[0].BlendOp = D3D11_BLEND_OP_MAX;
	blendDesc.RenderTarget[0].SrcBlend = D3D11_BLEND_ONE;
	blendDesc.RenderTarget[0].DestBlend = D3D11_BLEND_ONE;
	pd3dDevice->CreateBlendState(&blendDesc, &myDxState::g_pBSMaxOneOne);
	...
	if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &myDxState::g_pRSBackCullSolid))) return hr;


	rsDesc.FillMode = D3D11_FILL_WIREFRAME;
	rsDesc.CullMode = D3D11_CULL_NONE;
	if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &myDxState::g_pRSWireFrame)))
		return hr;
	rsDesc.FillMode = D3D11_FILL_SOLID;
	rsDesc.CullMode = D3D11_CULL_BACK;
	if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &myDxState::g_pRSSolid)))
		return hr;

	rsDesc.FillMode = D3D11_FILL_SOLID;
	rsDesc.CullMode = D3D11_CULL_NONE;
	if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &myDxState::g_pRSNoneCullSolid)))
		return hr;


	rsDesc.FillMode = D3D11_FILL_SOLID;
	rsDesc.CullMode = D3D11_CULL_FRONT;
	if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &myDxState::g_pRSFrontCullSolid)))
		return hr;
	...
	if (FAILED(hr = pd3dDevice->CreateRasterizerState(&rsDesc, &myDxState::g_pRSSlopeScaledDepthBias)))
		return hr;
